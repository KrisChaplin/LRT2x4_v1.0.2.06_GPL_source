...
static DEFINE_SPINLOCK(nk_pppol2tp_recv_core_lock);
...
static int pppol2tp_recv_core(struct sock *sock, struct sk_buff *skb)
{
	...
	spin_lock_bh(&nk_pppol2tp_recv_core_lock);
	/* Try to dequeue as many skbs from reorder_q as we can. */
	pppol2tp_recv_dequeue(session);
	spin_unlock_bh(&nk_pppol2tp_recv_core_lock);
	EXIT_FUNCTION;
	return 0;
...
static int pppol2tp_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < PPPOL2TP_HASH_SIZE; i++) {
		hlist_for_each_safe(walk, tmp, &tunnel->session_hlist[i]) {
			...
			seq_printf(m, "   %ld/%ld %ld/%ld/%ld %ld/%ld/%ld\n",
