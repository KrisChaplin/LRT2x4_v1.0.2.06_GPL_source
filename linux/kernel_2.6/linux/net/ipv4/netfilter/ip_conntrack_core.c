...
#ifdef CONFIG_NK_IP_CONNTRACK_ENHANCE
...
//				printk("nk_trans:index[%u]\n",BiosIndex);
				...
				for(pComparEntry=nat_ipsec_bc_ip_list[BiosIndex].start;pComparEntry;pComparEntry=pComparEntry->next){
					if((iphead->daddr==pComparEntry->local_bc)||(iphead->daddr==0xffffffff)){
...
int nk_translate_spiltdns_to_ipsec(struct sk_buff **pskb)
{
	...
	if(ipsec_split_dns_list.start){
		...
		if(iphead->protocol == IPPROTO_UDP)
		{
				...
				datalen = (*pskb)->len - iphead->ihl * 4;
				//dns size =  datalen - 25
				if(((sport!=53)  && (dport!=53)) || (  (dport==53)  &&(datalen > 89)) )
				{
					return 0;
				}	
				DNS =(char *)udphead+8;
				...
				if((*pskb)->dev == NULL)
				{
					printk(KERN_EMERG "====Frank  nk_translate_spiltdns_to_ipsec  (*pskb)->dev  is NULL  and it is fixed!!!! \n");
					return 0;
				}
				if((*pskb)->dev->name == NULL)
				{
					printk(KERN_EMERG "====Frank  nk_translate_spiltdns_to_ipsec  (*pskb)->dev->name  is NULL  and it is fixed!!!! \n");
					return 0;
				}
						...
						if( atomic_read(&cDnsSplitNum) >=MAX_DNS_SPLIT_HASH_NUM ){
							...
							for(jump=0;jump<DNS_SPLIT_HASH_SIZE;++jump){
								...
								if(split_hash_p){
									...
									kfree(split_hash_p);
									if(atomic_read(&cDnsSplitNum) > 0)
									{
										atomic_dec(&cDnsSplitNum);
									}
									else
										printk(KERN_EMERG "====Frank  nk_translate_spiltdns_to_ipsec  cDnsSplitNum is less than 1     and it is fixed!!!! \n");
								}
								spin_unlock(&(SplitdnsHashLock));
							}
						}
