 ...
 *
 * Version:	$Id: route.c 8333 2011-04-07 08:57:44Z max.yang $
 *
 * Authors:	Ross Biro
				  ...
				  struct rtable **result) 
{
	...
	/*description : when the result after routing is not ipsec, don't enter ipsec module */
	if(res->fi->fib_priority != NK_IPSEC_METRIC)
	{
		rth->u.dst.flags |= DST_NOPOLICY;
		rth->u.dst.flags |= DST_NOXFRM;
	}
				   ...
				   unsigned flags) 
{
	...
	/*purpose     : 0012950/14165  author : trenchen date : 2010-07-21                         */
	/*description : when the result after routing is not ipsec, don't enter ipsec module */
	if(res->fi && res->fi->fib_priority != NK_IPSEC_METRIC)
	{
		rth->u.dst.flags |= DST_NOPOLICY;
		rth->u.dst.flags |= DST_NOXFRM;
	}
