 ...
 *
 *	$Id: route.c 8826 2011-06-21 07:17:16Z paul.chen $
 *
...
/*purpose     : ipv6 author : paul date : 2011-06-21 */
/*description : support ipv6 loadbalance             */
struct rt6_info *ip6_route_select_equalize(struct rt6_info *old_rt)
{
	struct fib6_node *fn = old_rt->rt6i_node;
	struct rt6_info *new_rt = 0;
	struct rt6_info *temp_rt = 0;
	int w;

	/*for(temp_rt=old_rt;temp_rt;temp_rt=temp_rt->u.next){
		printk("rt[%x] weight[%u] power[%u] metric[%u]\n",temp_rt,temp_rt->weight,temp_rt->power,temp_rt->rt6i_metric);
	}*/

	if( old_rt->rt6i_metric == NK_EQUALIZERV6_METRIC ){
		//it is default route
		if(fn->weight > 1){
			if(fn->power == 0){
				temp_rt = fn->leaf;
				//refill power value
				fn->power = fn->weight;
				while(temp_rt && temp_rt->rt6i_metric == NK_EQUALIZERV6_METRIC){
					temp_rt->power = temp_rt->weight;
					temp_rt = temp_rt->u.next;
				}
			}

			//select default 
			w = jiffies % fn->power;
			for(temp_rt=old_rt; temp_rt && temp_rt->rt6i_metric == NK_EQUALIZERV6_METRIC; temp_rt = temp_rt->u.next){
				if( temp_rt->power && (w -= temp_rt->power) <= 0 ){
					//get it
					temp_rt->power--;
					fn->power--;
					new_rt = temp_rt;
...
restart:
	...
	/*purpose     : ipv6 author : paul date : 2011-06-21 */
	/*description : support ipv6 loadbalance             */
	if(fn->fn_bit ==0 && fn->weight > 1){
		/*printk("ip6_route_input:node weight[%u] power[%u]  oldrt[%x]\n",fn->weight,fn->power,rt);*/
		new_rt = ip6_route_select_equalize(rt);
		if(new_rt && new_rt != rt){
			rt = new_rt;
			/*printk("ip6_route_input:selec rt[%x]\n",rt);*/
		}
	}
#endif
	dst_hold(&rt->u.dst);
			      ...
			      struct in6_rtmsg *rtmsg)
{
	...
	/*purpose     : ipv6 author : paul date : 2011-06-21 */
	/*description : support ipv6 loadbalance             */
	if (rta[RTA_WEIGHT6-1]) {
		if (rta[RTA_WEIGHT6-1]->rta_len != RTA_LENGTH(4))
			return -EINVAL;
		memcpy(&rtmsg->rtmsg_weight6, RTA_DATA(rta[RTA_WEIGHT6-1]), 4);
	}
#endif
	return 0;
}
			 ...
			 int prefix, unsigned int flags)
{
	...
	/*purpose     : ipv6 author : paul date : 2011-06-21 */
	/*description : support ipv6 loadbalance             */
	RTA_PUT(skb, RTA_WEIGHT6, 4, &rt->weight);
#endif
