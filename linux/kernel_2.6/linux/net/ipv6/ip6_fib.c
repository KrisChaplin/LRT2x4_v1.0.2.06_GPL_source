 ...
 *
 *	$Id: ip6_fib.c 12738 2013-10-30 02:49:37Z chunru $
 *
...
/* purpose     : CONFIG_IPV6    author : ChunRu    date : 2013-10-29 */
/* description : fix kernel may crash when daddr not exist           */
	...
	/*purpose     : ipv6 author : paul date : 2011-06-21 */
	/*description : support ipv6 source loadbalance      */
#if 0
	...
	while(fn) {
		if(fn->subtree  || fn->fn_flags & RTN_RTINFO){
			struct rt6key *key;
			...
			if (ipv6_prefix_equal(&key->addr, args->addr, key->plen)) {
				...
				if (fn->subtree)
				{
					...
					if (narg->addr)
					{
						fn = fib6_lookup_1(fn->subtree, narg);
					}
				}
#endif
