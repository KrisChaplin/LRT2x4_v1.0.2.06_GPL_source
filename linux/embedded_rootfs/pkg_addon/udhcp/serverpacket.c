...
/* 2007/07/30 jane: bug fix dhcp status do not show client's host name */
#if 1
	if (!add_lease(packet.chaddr, packet.yiaddr, server_config.offer_time, usr, (u_int32_t)2)) {
#else
...
/* 2007/07/30 jane: bug fix dhcp status do not show client's host name */
#if 1
	add_lease(packet.chaddr, packet.yiaddr, lease_time_align, usr, (u_int32_t)4);
#else
