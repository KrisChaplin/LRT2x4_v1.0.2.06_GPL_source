 ...
 * Modified for PoPToP
 *
 * $Id: pptpgre.c 12798 2013-11-13 09:10:25Z fangchih $
 */
...
int decaps_gre(int fd, int (*cb) (int cl, void *pack, unsigned len), int cl)
{
	...
	if (PPTP_GRE_IS_S(ntoh8(header->flags))) {	/* payload present */
		...
		/* purpose     : 0014585 author : trenchen date : 2011-09-14               */
		/* description : the sequence of first gre packet be the starting sequence */
		/*if (seq == gre.seq_recv + 1) {*/
		if (gre.seq_recv == 0xFFFFFFFF || seq == gre.seq_recv + 1) {
			if (pptpctrl_debug)
