...
/*  if ( ioctl(s,SIOCSIFADDR,&ifr) == -1 )
  ...
  /* purpose     : CONFIG_IPV6    author : paul.chen    date : 2010-06-29 */
  /* description : Modify DHCP IPv4/IPv6 UI Release Button                */
  /*
  if ( ioctl(dhcpSocket,SIOCSIFFLAGS,&ifr) )
    syslog(LOG_INFO,"dhcpStop: ioctl SIOCSIFFLAGS: %m\n");
  */
  if ( ioctl(dhcpSocket,SIOCSIFADDR,&ifr) )
    syslog(LOG_INFO,"dhcpStop: ioctl SIOCSIFADDR: %m\n");
  close(dhcpSocket);
  if ( ReplResolvConf )
...
//		kd_doCommand(cmd, CMD_DELETE, ASH_DO_NOTHING, (char *)NULL);
	...
	sprintf(cmd, "ISP%d DNS1=0.0.0.0",iface);
	kd_doCommand(cmd, CMD_WRITE, ASH_DO_NOTHING, (char *)NULL);
	sprintf(cmd, "ISP%d DNS2=0.0.0.0",iface);
	kd_doCommand(cmd, CMD_WRITE, ASH_DO_NOTHING, (char *)NULL);
	sprintf(cmd, "ISP%d DNS3=0.0.0.0",iface);
	kd_doCommand(cmd, CMD_WRITE, ASH_DO_NOTHING, (char *)NULL);
