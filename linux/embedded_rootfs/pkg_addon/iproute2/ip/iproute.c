...
int print_route(const struct sockaddr_nl *who, struct nlmsghdr *n, void *arg)
{
	...
	/*purpose     : ipv6 author : paul date : 2011-06-21 */
	/*description : support ipv6 loadbalance             */
	if ( tb[RTA_WEIGHT6] && *(__u32*)RTA_DATA(tb[RTA_WEIGHT6]) )
		fprintf(fp, " weight %u ", *(__u32*)RTA_DATA(tb[RTA_WEIGHT6]));
#endif
...
int iproute_modify(int cmd, unsigned flags, int argc, char **argv)
{
	...
	while (argc > 0) {
		...
		/*purpose     : ipv6 author : paul date : 2011-06-21 */
		/*description : support ipv6 loadbalance             */
		} else if (matches(*argv, "6weight") == 0) {
			__u32 weight_v6;
			NEXT_ARG();
			if (get_u32(&weight_v6, *argv, 0))
				invarg("\"6weight\" value is invalid\n", *argv);
			addattr32(&req.n, sizeof(req), RTA_WEIGHT6, weight_v6);
#endif
