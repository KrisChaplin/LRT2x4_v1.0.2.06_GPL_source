...
char *xStrdup (char *string)
{
    ...
    if (tmp == (char *) NULL)
    {
        errorMsg("Error: mystrdup(): memory allocation problem\n");
        exit(0);
    }
...
/*
    if((strstr(buf, "missing")) || (strstr(buf, "Error")) || (strstr(buf, "fail")) || (strstr(buf, "error")) || (strstr(buf, "Could not"))) {
		sprintf(msg, "OPEN VPN: %s", buf);
		NK_LOG_VPN(LOG_ERR, msg);
    }
*/
    va_end(args);
...
TheMail *newTheMail(void)
{
    ...
    {
        errorMsg("initTheMail(): malloc failed\n");
        return ((TheMail *) NULL);
    }
...
Address *newAddress(void)
{
    ...
    if (a == (Address *) NULL)
    {
        errorMsg(" newAddress() malloc failed\n");
    }
...
int rfc822_date(time_t when,char *datebuf,int bufsiz)
{
    ...
    if (bufsiz < 64)
    {
        errorMsg("buffer size of date must be > 31, it is %d\n",bufsiz);
        return(-1);
    }
    ...
    */
    ...
    {
        errorMsg("UTC time offset %d is larger than one day",gmtoff);
        return(-1);
    }
