    ...
    */
    if (is_mime)
    {
        ...
        */
        ...
        (void) strcpy(mime_tmpfile,"/tmp/mailsendXXXXXX");
        ...
        /* handle mime */
        {
            ...
            for (al=attachment_list; al; al=al->next)
            {
                ...
                if (tfp == NULL)
                {
                     errorMsg("Could not open tmp file %s for writing (for %s)\n",mime_tmpfile,a->file_name);
                    return (-1);
                }
#endif /* HAVE_MKSTEMP */
                ...
                if (fp == (FILE *) NULL)
                {
                    errorMsg("Could not open tmp file %s for reading\n",mime_tmpfile);
                    return (-1);
                }
                while (fgets(mbuf,sizeof(mbuf)-1,fp))
                {
             ...
             int add_dateh)
{
    ...
    errorMsg("Sub: %s\n",sub);
    errorMsg("smtp: %s\n",smtp_server);
    errorMsg("smtp port: %d\n",smtp_port);
    errorMsg("domain: %s\n",helo_domain);
    errorMsg("attach file: %s\n",attach_file);
    errorMsg("txt_msg_file: %s\n",txt_msg_file);
    errorMsg("the_msg: %s\n",the_msg);
    errorMsg("is_mime: %d\n",is_mime);
    */
