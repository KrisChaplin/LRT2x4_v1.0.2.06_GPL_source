...
int main(int argc,char **argv)
{
            ...
            */
            ...
            case 's':
            {
                ...
                else if (strncmp("ssl", option + 1, 3) == 0)
                {
                    ...
                    g_do_ssl=1;
#else
                     errorMsg("Warning: '-ssl' not available, not compiled with OpenSSL\n");
#endif /* HAVE_OPENSSL */
                ...
                else if (strncmp("starttls",option+1,3) == 0)
                {
                    ...
                    g_do_starttls=1;
#else
                     errorMsg("Warning: '-starttls' not available, not compiled with OpenSSL\n");
            ...
            case 'V':
            {
                 errorMsg("mailsend Version: %.1024s\n",MAILSEND_VERSION);
#ifdef HAVE_OPENSSL
                 errorMsg("Compiled with %s\n",
                               SSLeay_version(SSLEAY_VERSION));
#else
                errorMsg("Not Compiled OpenSSL, some auth methods will be unavailable\n");
#endif /* ! HAVE_OPENSSL */
                exit(0);
                break;
            }
           ...
           case 't':
           {
                if (strncmp("to",option+1,1) == 0)
                {
                    if (*option == '-')
                    {
                        ...
                        if (i == argc)
                        {
                            errorMsg("Error: missing to addresses\n");
                            return (1);
                        }
                        to=argv[i];
            ...
            case 'r':
            {
                if (strncmp("rrr",option+1,3) == 0)
                {
                    if (*option == '-')
                    {
                        ...
                        if (i == argc)
                        {
                             errorMsg("Error: missing to addresses for -rrr\n");
                            return (1);
                        }
                ...
                else if (strncmp("rt",option+1,2) == 0)
                {
                    if (*option == '-')
                    {
                        ...
                        if (i == argc)
                        {
                             errorMsg("Error: missing to addresses for -rt\n");
                            return (1);
                        }
            ...
            default:
            {
                 errorMsg("Error: Unrecognized option: %s\n",
                               option);
                return (1);
            }
    ...
    {
         errorMsg("Options -ssl and -starttls are mutually exclusive\n");
        exit(1);
    }
#endif /* HAVE_OPENSSL */
    ...
    */
...
#endif /* WINNT */
    {
        ...
        if (g_do_ssl || g_do_starttls)
        {
            ...
            if (*g_userpass == '\0')
            {
                 ...
                 errorMsg("Or by environment variable SMTP_USER_PASS\n");
                exit(1);
            }
        }
    }
	...
	if(strstr(attach_file, ".ovpn")>0 )
	{
		if((p = strstr(attach_file, "client")) != NULL){
			NK_LOG_VPN(LOG_INFO,"OPEN VPN: %s sent successfully", p);
		}
		else{
			NK_LOG_VPN(LOG_INFO,"OPEN VPN: mail sent successfully");
		}
	}
	...
	{
		if((p = strstr(attach_file, "client")) != NULL){
			NK_LOG_VPN(LOG_INFO,"OPEN VPN: %s Could not send ", p);
		}
		else{
			NK_LOG_VPN(LOG_INFO,"OPEN VPN: Could not send mail");
		}
	}
    }
