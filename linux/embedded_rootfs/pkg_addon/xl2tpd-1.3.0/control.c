    ...
    case 0:
		...
		if (t->chal_them.challenge)
            ...
            if (gconfig.packet_dump)
                do_packet_dump (buf);
            if (gconfig.debug_state)
                l2tp_log (LOG_INFO, "%s: sending SCCRQ\n",
                     __FUNCTION__);
            control_xmit (buf);
        }
        else
...
/*					c->ourrws = c->lac->call_rws;
                ...
                if (gconfig.packet_dump)
                    do_packet_dump (buf);
                if (gconfig.debug_state)
                    l2tp_log (LOG_INFO, "%s: sending ICRQ\n", __FUNCTION__);
                control_xmit (buf);
            }
            else
            {                   /* jz: sending a OCRQ */
...
/*                                      c->ourrws = c->lac->call_rws;
    ...
    case SCCRQ:
        ...
        if (t->tid <= 0)
        {
			...
			l2tp_log (LOG_INFO,
                     "SCCRQ Peer did not specify assigned tunnel ID.  Closing.\n");					 
        ...
        if (!(t->lns = get_lns (t)))
        {
			...
			l2tp_log (LOG_INFO,
                     "SCCRQ Denied connection to unauthorized peer %s\n",
        ...
        if (t->fc < 0)
        {
			...
			l2tp_log (LOG_INFO,
                     "SCCRQ Peer did not specify framing capability.  Closing.\n");		 
        ...
        if ((!strlen (t->hostname)) && ((t->chal_us.state) || ((t->lns->challenge))))
        {
            ...
            if (DEBUG)
				...
				l2tp_log (LOG_INFO,
                     "Peer did not specify hostname.  Closing.\n");		 
		...
		free(t->chal_them.challenge);
            ...
            if (!(t->chal_them.challenge))
            {
                l2tp_log (LOG_INFO, "SCCRQ malloc failed\n");
            ...
            if (handle_challenge (t, &t->chal_them))
            {
                ...
                l2tp_log (LOG_INFO, "%s: No secret for '%s'\n", __FUNCTION__,
                     t->hostname);
        ...
        c->cnu = 0;
        if (gconfig.debug_state)
            l2tp_log (LOG_NOTICE, "%s: sending SCCRP\n", __FUNCTION__);
		sleep(2);
        control_xmit (buf);
        break;
    case SCCRP:
        ...
        if (t->fc < 0)
        {
			...
			l2tp_log (LOG_INFO,
                     "SCCRP Peer did not specify framing capability.  Closing.\n");		 
            set_error (c, VENDOR_ERROR, "Specify framing capability");
            c->needclose = -1;
            return -EINVAL;
        }
        ...
        if ((!strlen (t->hostname)) && ((t->chal_them.state) || ((t->chal_us.state))))
        {
			...
			l2tp_log (LOG_INFO,
                     "SCCRP Peer did not specify hostname.  Closing.\n");		 
            set_error (c, VENDOR_ERROR, "Specify your hostname");
            c->needclose = -1;
            return -EINVAL;
        }
		...
		l2tp_log (LOG_INFO,
                     "SCCRP Peer did not specify assigned tunnel ID.  Closing.\n");		 
            set_error (c, VENDOR_ERROR, "Specify your assigned tunnel ID");
            c->needclose = -1;
            return -EINVAL;
        }
        if (t->chal_them.state)
        {
            ...
            if (handle_challenge (t, &t->chal_them))
            {
                ...
                l2tp_log (LOG_INFO, "SCCRP No secret key for authenticating '%s'\n", t->hostname);
                c->needclose = -1;
                return -EINVAL;
            }
            if (memcmp
                (t->chal_them.reply, t->chal_them.response, MD_SIG_SIZE))
            {
                ...
                l2tp_log (LOG_INFO, "SCCRP Invalid authentication for host '%s'\n", t->hostname);
                c->needclose = -1;
                return -EINVAL;
            }
        }
        if (t->chal_us.state)
        {
            ...
            {
                l2tp_log (LOG_INFO, "SCCRP No secret for authenticating to '%s'\n", t->hostname);
        ...
        c->cnu = 0;
        if (gconfig.debug_state)
            l2tp_log (LOG_NOTICE, "%s: sending SCCCN\n", __FUNCTION__);
        control_xmit (buf);
    ...
    case SCCCN:
        if (t->chal_them.state)
        {
                ...
                (t->chal_them.reply, t->chal_them.response, MD_SIG_SIZE))
            {
                set_error (c, VENDOR_ERROR,
                           "Invalid challenge authentication");
                l2tp_log (LOG_INFO, "SCCCN Invalid authentication for host '%s'\n", t->hostname);
                c->needclose = -1;
                return -EINVAL;
            }
        }
    ...
    case StopCCN:
        if (t->qtid < 0)
        {
			...
			l2tp_log (LOG_INFO,
                     "StopCCN Peer tried to disconnect without specifying tunnel ID\n");		 
            return -EINVAL;
        }
       ...
       /* Work around bug in MSL2TP client */
		...
		if(!strcmp(t->self->errormsg,"goodbye!") || !strcmp(t->self->errormsg,"Server closing!") )
			l2tp_log (LOG_INFO,
...
/*		p->ourrws = p->lns->call_rws;
        ...
        if (p->cid < 0)
        {
			...
			l2tp_log (LOG_INFO,
                     "ICRQ Peer tried to initiate call without call ID\n");		 
...
/*    JLM	if (p->serno<1) {
        ...
        if (t->lns->assign_ip) {
            ...
            if (!p->addr)
            {
                ...
                call_close (p);
                l2tp_log (LOG_INFO, "ICRQ Out of IP addresses on tunnel %d!\n", t->tid);
                return -EINVAL;
            }
            reserve_addr (p->addr);
        }
...
/*		if (p->ourrws >=0)
        ...
        p->cnu = 0;
        if (gconfig.debug_state)
            l2tp_log (LOG_NOTICE, "%s: Sending ICRP\n", __FUNCTION__);
        control_xmit (buf);
        break;
    case ICRP:
        if (c->cid < 0)
        {
			...
			l2tp_log (LOG_INFO,
                     "ICRP Peer tried to negotiate ICRP without specifying call ID\n");		 
            c->needclose = -1;
            return -EINVAL;
        }
        c->state = ICCN;
 ...
 * don't ask for sequencing */
    ...
    case ICCN:
        ...
        {
            l2tp_log (LOG_INFO,
                 "ICCN Peer attempted ICCN on the actual tunnel, not the call");
            return -EINVAL;
        }
        if (c->txspeed < 1)
        {
            l2tp_log (LOG_INFO,
                 "ICCN Warning: Peer did not specify transmit speed\n");
	    ...
	    */
        ...
        {
            l2tp_log (LOG_INFO,
                 "ICCN Warning: Peer did not specify framing type\n");
             /* don't refuse the connection over this
            c->needclose = -1;
            return -EINVAL;
            */
        ...
        if (c->qcid < 0)
        {
            if (DEBUG)
					l2tp_log (LOG_DEBUG,
                     ...
                     __FUNCTION__);
           	l2tp_log (LOG_INFO,
                     "CDN Peer tried to disconnect without specifying call ID\n");
            return -EINVAL;
        }
        if (c == t->self)
        {
            ...
            if (!p)
            {
               if (DEBUG)
                    ...
                    l2tp_log (LOG_INFO,
                         "CDN Unable to determine call to be disconnected.\n");
                return -EINVAL;
            }
        }
        else {
