...
// <-------
    ...
    int x;
#endif
    int dnsNum,dnsidx,wanidx;
    char dnslist[4][20];
    char cmdBuf[80],buf[80];
    ...
    if(!nk_Isclient)
        stropt[pos++] = strdup ("l2tpd");
    else
    {
        stropt[pos++] = strdup ("l2tpc");
    }
    while (p)
    {
    ...
    if (c->pppd > 0)
    {
        l2tp_log(LOG_INFO, "%s: PPP already started on call!\n", __FUNCTION__);
        return -EINVAL;
    }
    if (c->fd > -1)
    {
    ...
    if (kernel_support)
    {
	   ...
	   // ------>
		...
		if(!nk_Isclient)
		{
			...
			for (dnsidx = 0; dnsidx < 3; dnsidx++) 
			{
				...
				{
#endif
					sprintf(cmdBuf, "ISP%d GATEWAY", wanidx+1);	
					kd_doCommand(cmdBuf, CMD_PRINT, ASH_DO_NOTHING, buf);	
					if (!strcmp(buf, "0.0.0.0"))
						continue;
					sprintf(cmdBuf, "WAN%d USERSPECIALDNS", wanidx+1);	
					kd_doCommand(cmdBuf, CMD_PRINT, ASH_DO_NOTHING, buf);	
					if (!strcmp(buf, "NO"))	// not USERSPECIALDNS, ISP DNS
						sprintf(cmdBuf, "ISP%d DNS%d", wanidx+1,dnsidx+1);	
					else
						sprintf(cmdBuf, "WAN%d DNS%d", wanidx+1,dnsidx+1);
					kd_doCommand(cmdBuf, CMD_PRINT, ASH_DO_NOTHING, buf);
					if (strcmp(buf, "0.0.0.0") ) 
					{
						int i;
						for (i=0; i<4; i++)
							if (dnslist[i][0]!=NULL && !strcmp(dnslist[i], buf)) break; 
						if (i!=4) continue;
						for (i=0; i<4; i++)
							if (dnslist[i][0] == 0) break;
						sprintf(dnslist[i], buf);
						stropt[pos++] = strdup ("ms-dns");
						stropt[pos] = (char *) malloc (20); 
						snprintf (stropt[pos], 20, "%s", buf);
						pos++;
						dnsNum++;
					}
				}
       ...
       snprintf (stropt[pos], 10, "%d", fd2);
        pos++;
	 // <-------  add for xl2tpd-lns patch . By Jacky.Chen 2012.06.13
	if (c->container->lns) { 
        ...
        stropt[pos] = (char *) malloc (10); 
        snprintf (stropt[pos], 10, "%d", c->cid); 
            pos++; 
	}
       // ------>
        stropt[pos] = NULL;
...
#endif
    {
        if ((c->fd = pty_get (tty, sizeof(tty))) < 0)
        {
        ...
        *(c->oldptyconf) = ptyconf;
        ...
        fd2 = open (tty, O_RDWR);
        if (fd2 < 0) {
            l2tp_log (LOG_ERR, "unable to open tty %s, cannot start pppd", tty);
            return -EINVAL;
        }
...
// ------>
 #ifdef USE_KERNEL 
                if (kernel_support) { 
                    close(st->u_fd); /* tunnel UDP fd */ 
                    close(st->m_fd); /* tunnel PPPoX fd */ 
                } else 
#endif
// ------>
                close (sc->fd);
...
void destroy_tunnel (struct tunnel *t)
{
    ...
    if (t->lac)
    {
        ...
        if (t->lac->redial && (t->lac->rtimeout > 0) && !t->lac->rsched &&
            t->lac->active)
        {
            l2tp_log (LOG_NOTICE, "Will redial in %d seconds\n",
                 t->lac->rtimeout);
                          ...
                          struct lns *lns)
{
	...
	l2tp_log (LOG_INFO, "Try to Connecting host %s, port %d\n", host, ntohs (port));	 
...
void magic_lac_dial (void *data)
{
    ...
    {
        l2tp_log (LOG_ERR, "%s: maximum retries exceeded.\n", __FUNCTION__);
//#ifdef CONFIG_NK_L2TP_CLIENT
        sprintf(cmd,"WAN%d",cwanidx);
        kd_doCommand(cmd, NULL, ASH_DO_L2TP_REDIAL, (char *)NULL);
//#endif		
        return;
    }
    if (!lac)
...
void daemonize() {
    ...
    if((pid = fork()) < 0) {
        l2tp_log(LOG_ERR, "%s: Unable to fork ()\n",__FUNCTION__);
        close(server_socket);
        exit(1);
    }
    else if (pid)
    ...
    if (i != 0) {
        l2tp_log(LOG_WARNING, "Redirect of stdin to /dev/null failed\n");
    } else {
        if (dup2(0, 1) == -1)
            l2tp_log(LOG_WARNING, "Redirect of stdout to /dev/null failed\n");
        if (dup2(0, 2) == -1)
            l2tp_log(LOG_WARNING, "Redirect of stderr to /dev/null failed\n");
    }
#endif
}
...
void init (int argc,char *argv[])
{
    ...
    l2tp_log (LOG_NOTICE, "xl2tpd version " SERVER_VERSION " started on %s PID:%d\n",
         hostname, getpid ());
    l2tp_log (LOG_NOTICE,
         "Written by Mark Spencer, Copyright (C) 1998, Adtran, Inc.\n");
    l2tp_log (LOG_NOTICE, "Forked by Scott Balmos and David Stipp, (C) 2001\n");
    l2tp_log (LOG_NOTICE, "Inherited by Jeff McAdams, (C) 2002\n");
    l2tp_log (LOG_NOTICE, "Forked again by Xelerance (www.xelerance.com) (C) 2006\n");
    listenaddr.s_addr = gconfig.listenaddr;
    l2tp_log (LOG_NOTICE, "Listening on IP address %s, port %d\n",
...
int main (int argc, char *argv[])
{
    ...
    if(!nk_Isclient)
    {
        sprintf(cmd, "PPTP_SERVER PID=%d",getpid());
        kd_doCommand(cmd, CMD_WRITE, ASH_DO_NOTHING, (char *)NULL);
    }
    else
    {
        sprintf(cmd, "L2TP%d XL2TPDPID=%d",cwanidx,getpid());
        kd_doCommand(cmd, CMD_WRITE, ASH_DO_NOTHING, (char *)NULL);
    }
