...
{ 
  ...
  this.nodeImg = 0;
  this.isLastNode = 0;
  this.iconSrc = ICONPATH + "navarrow_wht_expanded.png";
  this.iconSrcClosed = ICONPATH + "navarrow_wht_collapsed.png";
...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a style=\"color:#ffffff\"href='javascript:clickOnNode(\""+this.getID()+"\")'>" 
  else 
    auxEv = "<a style=\"color:#ffffff\">" 
  ...
  {
    nodeName = ICONPATH + '';
   // leftSide = leftSide + "<td valign=center>"+"<a style=\"color:#ffffff\">" +"<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "'  alt='' width=0 height=0 border=0></a></td>"
 }
...
if(this.getID()!=foldersTree.getID()){
  docW = docW + "<tr height=\"24\">" + leftSide + "<td valign=center>";
...
{ 
  ...
  { 
    ...
    if (isTextLink) {
	if (USEFRAMES)
	  docW = docW + "<a style=\"color:#ffffff\" href='" + this.hreference + "' TITLE='"+this.desc+"' TARGET=\"contentiframe\" "
	else
	  docW = docW + "<a style=\"color:#ffffff\" href='" + this.hreference + "' TARGET=_top "
    }
    else
	docW = docW + "<a style=\"color:#ffffff\" href='javascript:void(0)' TARGET=\"contentiframe\" "
    ...
    docW = docW + "<a style=\"color:#ffffff\">" 
...
function finalizeCreationOfItem(itemArray)
{
  ...
  itemArray.iconSrc = ICONPATH + "navarrow_wht.png" 
...
{ 
  ...
  docW = docW + "<tr height=\"24\">" + leftSide + "<td valign=top>"

  if (USEICONS)
      docW = docW + "<a style=\"color:#ffffff\" " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' alt='open topic' border=0>" + "</a>"
  else
	  if (this.prependHTML == "")
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a style=\"color:#ffffff\"" + fullLink + " title=\""+this.desc+"\" id=\"itemTextLink"+this.id+"\">&nbsp;&nbsp;" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=center background=" + ICONPATH + "ftv2blank.gif><img src='" + ICONPATH + "ftv2blank.gif' alt='' width=10 height=14></td>"
		}
		if (leftSideCoded.charAt(i) == "0")
		{
			retStr = retStr + "<td valign=center><img src='" + ICONPATH + "ftv2blank.gif' alt='' width=10 height=14></td>"
		}
	}
	return retStr
}
...
//	auxImg.src = ICONPATH + "ftv2lastnode.gif";
	...
	auxImg.src = ICONPATH + "navarrow_wht_collapsed.png";
	auxImg.src = ICONPATH + "navarrow_wht_expanded.png";
	auxImg.src = ICONPATH + "navarrow_wht.png";
	auxImg.src = ICONPATH + "";
}
