 ...
 * for more details.
 *
 * RCSID $Id: spdb.c 12119 2013-07-09 13:59:35Z dio.li $
 */
		   ...
		   , struct state *st)
{
	...
	if (no_trans_left == 0)
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] number of Transform Payloads disagrees with Oakley Proposal Payload");
	    return BAD_PROPOSAL_SYNTAX;
	}

	if(!in_struct(&trans, &isakmp_isakmp_transform_desc, proposal_pbs, &trans_pbs)){
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] fail to parse ISAKMP Transform Payloads Oakley Proposal Payload");
		return BAD_PROPOSAL_SYNTAX;
	}
	attr_start = trans_pbs.cur;
	attr_len = pbs_left(&trans_pbs);
	...
	while (pbs_left(&trans_pbs) != 0)
	{
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] repeated %s attribute in Oakley Transform %u"
...
/* Encounter: choose what we want, not what we have */
	    ...
	    if (ugh != NULL)
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] %s.  Attribute %s"
	...
	if (ugh == NULL)
	{
		 ...
		 * reviewer : 	Jacky
		 * date	:	2012-05-16
		 * description:	We should synchronize the lifetime with outside client
		 */
		 if (st->st_connection && (st->st_connection->spd.that.id.kind == ID_FQDN || st->st_connection->spd.that.id.kind == ID_USER_FQDN)) 	 
		 {
		 	if (ta.life_seconds >0 && ta.life_seconds != st->st_connection->sa_ike_life_seconds)
		 		st->st_connection->sa_ike_life_seconds = ta.life_seconds;			
		 }		
         return NOTHING_WRONG;
	}
	...
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] unexpected %s payload in Oakley Proposal"
		, enum_show(&payload_names, proposal->isap_np));
	    return BAD_PROPOSAL_SYNTAX;
	}
    }
    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] no acceptable Oakley Transform");
...
, struct state *st)	/* current state object */
{
    ...
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Transform Numbers in Proposal are not monotonically increasing");
	return FALSE;
    }
	...
	case ISAKMP_NEXT_T:
	    if (is_last)
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Proposal Payload has more Transforms than specified");
		return FALSE;
	    }
	    break;
	case ISAKMP_NEXT_NONE:
	    if (!is_last)
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Proposal Payload has fewer Transforms than specified");
		return FALSE;
	    }
	    break;
	default:
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] expecting Transform Payload, but found %s in Proposal"
    ...
    *attrs = null_ipsec_trans_attrs;
	...
	if (LHAS(seen_attrs, a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK))
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] repeated %s attribute in IPsec Transform %u"
	...
	if (vdesc != NULL)
	{
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] invalid value %u for attribute %s in IPsec Transform"
	...
	switch (a.isaat_af_type)
	{
		...
		if (LHAS(seen_durations, val))
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] attribute SA_LIFE_TYPE value %s repeated in message"
		...
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] SA_LIFE_DURATION IPsec attribute not preceded by SA_LIFE_TYPE attribute");
		    return FALSE;
		}
		...
		if (is_ipcomp)
		{
		    ...
		    ipcomp_inappropriate = FALSE;
		    loglog(RC_COMMENT
			, "[Tunnel Authorize Fail] IPCA (IPcomp SA) contains GROUP_DESCRIPTION."
			"  Ignoring inapproprate attribute.");
		}
		pfs_group = lookup_group(val);
		if (pfs_group == NULL)
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] only OAKLEY_GROUP_MODP1024 and OAKLEY_GROUP_MODP1536 supported for PFS");
		    return FALSE;
		}
		break;
...
#ifndef I_KNOW_TRANSPORT_MODE_HAS_SECURITY_CONCERN_BUT_I_WANT_IT
		    loglog(RC_LOG_SERIOUS
			, "[Tunnel Authorize Fail] NAT-Traversal: Transport mode disabled due to security concerns");
		    return FALSE;
#endif
		case ENCAPSULATION_MODE_UDP_TUNNEL_DRAFTS:
		    ...
		    {
			loglog(RC_LOG_SERIOUS
			    , "[Tunnel Authorize Fail] %s must only be used with old IETF drafts"
			    , enum_name(&enc_mode_names, val));
			return FALSE;
		    }
		    ...
		    {
			loglog(RC_LOG_SERIOUS
			    , "[Tunnel Authorize Fail] %s must only be used if NAT-Traversal is detected"
			    , enum_name(&enc_mode_names, val));
			return FALSE;
		    }
		    break;
...
#ifndef I_KNOW_TRANSPORT_MODE_HAS_SECURITY_CONCERN_BUT_I_WANT_IT
			loglog(RC_LOG_SERIOUS
			    , "[Tunnel Authorize Fail] NAT-Traversal: Transport mode disabled due "
			...
			{
			    loglog(RC_LOG_SERIOUS
				, "[Tunnel Authorize Fail] %s must only be used with NAT-T RFC"
				, enum_name(&enc_mode_names, val));
			    return FALSE;
			}
			else
		    ...
		    default:
			loglog(RC_LOG_SERIOUS
			    , "[Tunnel Authorize Fail] unknown ENCAPSULATION_MODE %d in IPSec SA", val);
			return FALSE;
		}
		break;
...
#endif
	    default:
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] unsupported IPsec attribute %s"
		    , enum_show(&ipsec_attr_names, a.isaat_af_type));
		return FALSE;
	}
	if (ipcomp_inappropriate)
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec attribute %s inappropriate for IPCOMP"
		, enum_show(&ipsec_attr_names, a.isaat_af_type));
	    return FALSE;
	}
    }
	...
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] GROUP_DESCRIPTION inconsistent with that of %s in IPsec SA"
    ...
    if (LHAS(seen_attrs, SA_LIFE_DURATION))
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] SA_LIFE_TYPE IPsec attribute not followed by SA_LIFE_DURATION attribute in message");
	return FALSE;
    }
	...
	else
	{
	     ...
	     */
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec Transform must specify ENCAPSULATION_MODE");
	    return FALSE;
	}
    }
    ...
    struct state *st)		/* current state object */
{
    ...
    if (sa->isasa_doi != ISAKMP_DOI_IPSEC)
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Fail] Unknown or unsupported DOI %s", enum_show(&doi_names, sa->isasa_doi));
	/* XXX Could send notification back */
	return DOI_NOT_SUPPORTED;
    }
    ...
    if (ipsecdoisit != SIT_IDENTITY_ONLY)
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Fail] unsupported IPsec DOI situation (%s)"
	    , bitnamesof(sit_bit_names, ipsecdoisit));
	/* XXX Could send notification back */
	return SITUATION_NOT_SUPPORTED;
    }
	...
	do {
		...
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec Proposal with improper CPI size (%u)"
		...
		case IPCOMP_DEFLATE:
		    ...
		    {
			loglog(RC_LOG_SERIOUS
			    , "[Tunnel Authorize Fail] IPsec Proposal contains well-known CPI that I cannot uniquify");
			return INVALID_SPI;
		    }
		    break;
		default:
		    ...
		    {
			loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec Proposal contains CPI from non-negotiated range (0x%lx)"
		...
		if (next_proposal.isap_spisize != IPSEC_DOI_SPI_SIZE)
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec Proposal with improper SPI size (%u)"
			, next_proposal.isap_spisize);
		    return INVALID_SPI;
		}
		...
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec Proposal contains invalid SPI (0x%lx)"
			, (unsigned long) ntohl(next_spi));
		    return INVALID_SPI;
		}
	    }
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec Proposal contains no Transforms");
		return BAD_PROPOSAL_SYNTAX;
	    }
		...
		if (ah_seen)
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec SA contains two simultaneous AH Proposals");
		    return BAD_PROPOSAL_SYNTAX;
		}
		ah_seen = TRUE;
		...
		if (esp_seen)
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec SA contains two simultaneous ESP Proposals");
		    return BAD_PROPOSAL_SYNTAX;
		}
		esp_seen = TRUE;
		...
		if (ipcomp_seen)
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] IPsec SA contains two simultaneous IPCOMP Proposals");
		    return BAD_PROPOSAL_SYNTAX;
		}
		ipcomp_seen = TRUE;
	    ...
	    default:
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] unexpected Protocol ID (%s) in IPsec Proposal"
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] unexpected in Proposal: %s"
	...
	if (ah_seen)
	{
		...
		switch (ah_attrs.auth)
		{
			...
			loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] AUTH_ALGORITHM attribute missing in AH Transform");
			return BAD_PROPOSAL_SYNTAX;
...
//		break;	/* we seem to be happy */
	...
	if (ipcomp_seen)
	{
	    ...
	    if (!can_do_IPcomp)
	    {
			plog("[Internal Error] compression proposed by %s, but kernel does not support IPCOMP"
				, ip_str(&c->spd.that.host_addr));
			continue;
	    }
	    ...
	    {
		plog("[Tunnel Authorize Fail] illegal proposal: bare IPCOMP used with well-known CPI");
		return BAD_PROPOSAL_SYNTAX;
	    }
		...
		{
		    plog("[Tunnel Authorize Fail] illegal proposal: IPCOMP well-known CPI disagrees with transform");
		    return BAD_PROPOSAL_SYNTAX;
		}
	...
	if (esp_seen)
	{
		...
		if (st->st_connection && (st->st_connection->spd.that.id.kind == ID_FQDN || st->st_connection->spd.that.id.kind == ID_USER_FQDN)) 	 
		{
			if (esp_attrs.life_seconds >0 && esp_attrs.life_seconds != st->st_connection->sa_ipsec_life_seconds)
				st->st_connection->sa_ipsec_life_seconds = esp_attrs.life_seconds;			
		}
	}
    ...
    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] no acceptable Proposal in IPsec SA");
    return NO_PROPOSAL_CHOSEN;
}
