 ...
 * for more details.
 *
 * RCSID $Id: kernel.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
/* 
* purpose:	Synchronization with openswan-2.6.23 patch
* reviewer :	Dio.Li
* date	:	2011-11-21
* description:	Fix for spurious kernel acquires landing us wrongly into opportunistic code
*/
...
, unsigned int op, const char *opname)
{
    ...
    char buf2[256];
    /* purpose		:	fix pluto disapear problem , mantis id: 12484
     * author		:	frank.chang
     * date:		:	2010-05-27
     * description	:	fix pluto disapear problem , mantis id: 12484
     */
    ...
    if (proto == SA_INT)
    {
    	...
    	{
    	 	plog("eroute_connection:  peer is NULL");
    	}
    	else
    	{
	   ...
	   , unsigned op, const char *opname)
{
    ...
    if (sr->routing == RT_ROUTED_PROSPECTIVE && op == ERO_REPLACE)
    {
	(void) raw_eroute(&sr->that.host_addr, &sr->that.client
		      , &sr->this.host_addr, &sr->this.client
		      , htonl(st->st_tunnel_in_spi), inner_proto, inner_satype
		      , 0, proto_info + i, 0
...
setup_half_ipsec_sa(struct state *st, bool inbound)
{
	...
	*said_next = said;
	 ...
	 * reviewer	: Jacky
	 * date		: 2012-06-14
	 * description :  DONT_REKEY Tunnel doesn't need hard lifetime to detect
	 */
	if ((c->policy & POLICY_DONT_REKEY) == LEMPTY)
		said_next->life_time = (u_int64_t) c->sa_ipsec_life_seconds;
	...
	switch (st->st_ipcomp.attrs.transid)
	{
	    ...
	    default:
		loglog(RC_LOG_SERIOUS, "[Internal Error] IPCOMP transform %s not implemented"
	...
	for (ei = esp_info; ; ei++)
	{
		 ...
		 */
		loglog(RC_LOG_SERIOUS, "[Internal Error] ESP transform %s / auth %s not implemented yet"
	...
	if (key_len)
	{
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Internal Error] ESP transform %s passed key_len=%d > %d",
		    enum_name(&esp_transformid_names, st->st_esp.attrs.transid),
	...
	default:
	    loglog(RC_LOG_SERIOUS, "[Internal Error] %s not implemented yet"
...
install_inbound_ipsec_sa(struct state *st)
{
	...
	for (;;)
	{
	    ...
	    loglog(RC_LOG_SERIOUS, "[Internal Error] route to peer's client conflicts with \'%s\' %s; releasing old connection to free the route"
...
install_ipsec_sa(struct state *st, bool inbound_also USED_BY_KLIPS)
{
     ...
     * reviewer :	Max.Yang
     * date	:	2011-11-21
     * description:	Remote host might send the same SPI => setup_half_ipsec_sa() outbound SA failed cause always install inbound SA
     */
...
was_eroute_idle(struct state *st, time_t idle_max, time_t *idle_time)
{
	 ...
	 * author	:	Dio.Li
	 * date	:	2013-03-12
	 * description:	Record external inbound traffic bytes to detect idle situation.
	 */
#if 0
...
#else
	get_sa_info(st, TRUE, &bytes, &use_time);
	if (bytes > 0 && bytes > st->st_inbound_bytes)
	{
		ret = FALSE;
		st->st_inbound_bytes = bytes;
		*idle_time = 0;
	}	
	else
	{
		*idle_time = time(NULL)  - st->st_last_dpd;
		ret = *idle_time >= EVENT_RETRANSMIT_DELAY_0;
	}
#endif	
    }
    else 
