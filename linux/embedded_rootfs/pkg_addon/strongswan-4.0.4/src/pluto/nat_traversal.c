 ...
 * for more details.
 *
 * RCSID $Id: nat_traversal.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
void nat_traversal_natoa_lookup(struct msg_digest *md)
{
    ...
    if (!st || !md->iface)
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] NAT-Traversal: assert failed %s:%d"
		, __FILE__, __LINE__);
	return;
    }
    ...
    DBG(DBG_NATT,
	DBG_log("[Tunnel Negotiation Info] NAT-Traversal: received %d NAT-OA.", i)
    )
    ...
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] NAT-Traversal: received %d NAT-OA. "
	...
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] NAT-Traversal: received IPv4 NAT-OA "
	...
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] NAT-Traversal: received IPv6 NAT-OA "
			"with invalid IP size (%d)", (int)pbs_left(&p->pbs));
	    return;
	}
	break;
    default:
	loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] NAT-Traversal: "
    ...
    if (isanyaddr(&ip))
	loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] NAT-Traversal: received %%any NAT-OA...");
    else
	st->nat_oa = ip;
}
	...
	struct state *st)
{
    ...
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] NAT-Traversal: assert failed %s:%d"
			, __FILE__, __LINE__);
	return FALSE;
    }
	...
	break;
    default:
	loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] NAT-Traversal: "
