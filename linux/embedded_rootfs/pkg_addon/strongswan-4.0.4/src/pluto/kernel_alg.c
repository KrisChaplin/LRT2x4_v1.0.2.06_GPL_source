 ...
 * for more details.
 *
 * RCSID $Id: kernel_alg.c 12324 2013-08-07 05:30:03Z dio.li $
 */
...
kernel_alg_esp_ok_final(u_int ealg, u_int key_len, u_int aalg, struct alg_info_esp *alg_info)
{
	...
	if (alg_info)
	{
	    ...
	    {
		/* debug */
		loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Info] esp_ealg_id=%d-%d,esp_ealg_keylen=%d, key_len=%d,esp_aalg_id=%d-%d.",
		...
		&&  esp_info->esp_aalg_id == aalg)
		{
			...
			loglog(RC_LOG_SERIOUS
			    , "[Tunnel Negotiation Info] You should NOT use insecure ESP algorithms [%s (%d)]!"
	...
	plog("[Tunnel Authorize Fail] IPSec Transform [%s (%d), %s] refused due to %s",
		enum_name(&esp_transformid_names, ealg), key_len,
