 ...
 * for more details.
 *
 * RCSID $Id: timer.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
/* Max add for dont rekey bug 20110316   */
#if 1    
    if(ev->ev_type == EVENT_SA_REPLACE)
    {
      if(st->st_connection->policy & POLICY_DONT_REKEY)
	  ev->ev_type = EVENT_SA_REPLACE_IF_USED;
    }
#endif
...
handle_timer_event(void)
{
	...
	case EVENT_RETRANSMIT:
		...
		/*
			purpose : Synchronize the patch from openswan-2.6.24
			author : Dio.Li
			date : 2010-06-03
		*/
		...
		if (delay != 0)
		{
		    if(st->st_connection->newest_ipsec_sa != SOS_NOBODY && st->st_connection->newest_ipsec_sa > st->st_serialno)
		    {	
				delete_state(st);
				break;	 
		    }			
		...
		else
		{
			...
			, "[Tunnel Negotiation Fail] max number of retransmissions (%d) reached %s%s"
			, st->st_retransmit
			...
			snprintf(story, sizeof(story), try_limit == 0
			    ? "[Tunnel Negotiation Info] starting keying attempt %ld of an unlimited number"
			    : "[Tunnel Negotiation Info] starting keying attempt %ld of at most %ld"
			    , try, try_limit);
	    ...
	    break;
	 /* purpose : VPN/dontrekey   author : lucy.jiang	date : 2010-06-28  */
	 /* description : when the event is EVENT_SA_REPLACE_IF_USED,don't do nothing . */
	case EVENT_SA_REPLACE_IF_USED:
		event_schedule(EVENT_SA_REPLACE_IF_USED, PLUTO_SA_LIFE_DURATION_DEFAULT/2, st);
		break;
