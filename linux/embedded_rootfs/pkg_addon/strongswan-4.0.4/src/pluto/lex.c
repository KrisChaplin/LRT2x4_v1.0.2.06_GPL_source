 ...
 * for more details.
 *
 * RCSID $Id: lex.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
shift(void)
{
    ...
    *p = flp->under;
	...
	//case '\'':
		...
		if (p == NULL)
		{
		    loglog(RC_LOG_SERIOUS, "[Internal Error] \'%s\' line %d: unterminated string"
...
flushline(const char *m)
{
    ...
    {
	if (m != NULL)
	    loglog(RC_LOG_SERIOUS, "[Internal Error] \'%s\' line %d: %s", flp->filename, flp->lino, m);
	do ; while (shift());
	return FALSE;
    }
}
