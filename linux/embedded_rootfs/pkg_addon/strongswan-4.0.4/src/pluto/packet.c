 ...
 * for more details.
 *
 * RCSID $Id: packet.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
out_raw(const void *bytes, size_t len, pb_stream *outs, const char *name)
{
    ...
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] not enough room left to place %lu bytes of %s in %s"
...
out_zero(size_t len, pb_stream *outs, const char *name)
{
    if (pbs_left(outs) < len)
    {
	loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] not enough room left to place %s in %s", name, outs->name);
	return FALSE;
    }
    else
    {
