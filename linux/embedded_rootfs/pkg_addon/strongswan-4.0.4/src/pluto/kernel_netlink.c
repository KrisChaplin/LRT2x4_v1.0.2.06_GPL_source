 ...
 * for more details.
 *
 * RCSID $Id: kernel_netlink.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
, const char *description, const char *text_said)
{
    ...
    {
	log_errno((e
	    , "[Internal Error] netlink write() of %s message"
	      " for %s %s failed"
	...
	if (r < 0)
	{
	    ...
	    log_errno((e
		, "[Internal Error] netlink recvfrom() of response to our %s message"
		  " for %s %s failed"
		, sparse_val_show(xfrm_type_names, hdr->nlmsg_type)
		, description, text_said));
	    return FALSE;
	...
	{
	    plog("[Internal Error] netlink read truncated message: %ld bytes; ignore message"
		, (long) r);
	    continue;
	}
	else if (addr.nl_pid != 0)
	{
	    /* not for us: ignore */
	    DBG(DBG_KLIPS,
		DBG_log("[Internal Error] ignoring %s message from process %u"
	...
	{
	    DBG(DBG_KLIPS,
		DBG_log("[Internal Error] ignoring out of sequence (%u/%u) message %s"
		    , rsp.n.nlmsg_seq, seq
		    , sparse_val_show(xfrm_type_names, rsp.n.nlmsg_type)));
	    continue;
	}
    ...
    {
	loglog(RC_LOG_SERIOUS
	    , "[Internal Error] netlink response for %s %s included errno %d: %s"
	    , description, text_said
	    , -rsp.e.error
...
netlink_policy(struct nlmsghdr *hdr, bool enoent_ok, const char *text_said)
{
    ...
    loglog(RC_LOG_SERIOUS
	, "[Internal Error] netlink %s response for flow %s included errno %d: %s"
	, sparse_val_show(xfrm_type_names, hdr->nlmsg_type)
	, text_said
	, error
	, strerror(error));
...
netlink_add_sa(const struct kernel_sa *sa, bool replace)
{
    ...
    req.p.lft.hard_add_expires_seconds = sa->life_time;
...
retry:
	...
	{
	    get_cpi_bug = 1;
	    plog("netlink_get_spi: Enabling workaround for"
