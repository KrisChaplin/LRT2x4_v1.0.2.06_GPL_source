 ...
 * for more details.
 *
 * RCSID $Id: demux.c 12119 2013-07-09 13:59:35Z dio.li $
 */
...
#else
		    plog((sender != NULL) + "~"
			"[Tunnel Negotiation Fail] asynchronous network error report on %s"
			"%s"
			"cause : %s"
			" [errno %lu, origin %s"
			"]"
    ...
    if (err)
    {
       /* do not log NAT-T Keep Alive packets */
        if (streq(where, "NAT-T Keep Alive"))
 	    return FALSE;
	log_errno((e, "[Tunnel Negotiation Fail] sendto on %s to %s:%u failed in %s"
	    , c->interface->rname
	    , ip_str(&c->spd.that.host_addr)
	    , (unsigned)c->spd.that.host_port
	    , where));
...
read_packet(struct msg_digest *md)
{
	...
	if (packet_len == -1)
	{
		...
		&& errno == ECONNREFUSED)
		{
			 ...
			 */
			plog("[Tunnel Negotiation Fail] some IKE message we sent has been rejected with ECONNREFUSED (kernel supplied no details)");
		}
		else if (from_ugh != NULL)
		{
			log_errno((e, "[Tunnel Negotiation Fail] recvfrom on %s failed; Pluto cannot decode source sockaddr in rejection: %s"
				, ifp->rname, from_ugh));
		}
		else
		{
			log_errno((e, "[Tunnel Negotiation Fail] recvfrom on %s from %s:%u failed"
				, ifp->rname
	...
	{
		plog("[Tunnel Negotiation Fail] recvfrom on %s returned malformed source sockaddr: %s"
			, ifp->rname, from_ugh);
		free(buffer);
		return FALSE;
	}
	...
	if (ifp->ike_float == TRUE)
	{
		...
		{
			plog("[Tunnel Negotiation Fail] recvfrom %s:%u too small packet (%d)"
		...
		if (non_esp != 0)
		{
			plog("[Tunnel Negotiation Fail] recvfrom %s:%u has no Non-ESP marker"
...
#define SEND_NOTIFICATION(t) { \
    ...
    {
	plog("[Tunnel Authorize Fail] size (%u) differs from size specified in ISAKMP HDR (%u)"
...
//#ifdef CONFIG_IPEC_AGGR_MODE
	...
	{
	    plog("[Tunnel Authorize Fail] Message ID was 0x%08lx but should be zero in Main Mode",
	...
	{
	    plog("[Tunnel Authorize Fail] Initiator Cookie must not be zero in Main Mode message");
	...
	if (is_zero_cookie(md->hdr.isa_rcookie))
	{
	    ...
	    {
		plog("[Tunnel Authorize Fail] initial Main Mode message is invalid:"
	...
	else
	{
		...
		if (st == NULL)
		{
		    plog("[Tunnel Negotiation Fail] Main Mode message is part of an unknown exchange");
...
//    case ISAKMP_XCHG_AGGR:
	...
	if (md->hdr.isa_flags & ISAKMP_FLAG_ENCRYPTION)
	{
	    if (st == NULL)
	    {
		plog("[Tunnel Authorize Fail] Informational Exchange is for an unknown (expired?) SA");
		/* XXX Could send notification back */
		return;
	    }
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] encrypted Informational Exchange message is invalid"
	    ...
	    if (md->hdr.isa_msgid == MAINMODE_MSGID)
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Informational Exchange message is invalid because"
	...
	{
	    if (st != NULL && IS_ISAKMP_AUTHENTICATED(st->st_state))
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Informational Exchange message"
		    " must be encrypted");
		/* XXX Could send notification back */
		return;
	    }
	...
	if (is_zero_cookie(md->hdr.isa_icookie))
	{
	    plog("[Tunnel Authorize Fail] Quick Mode message is invalid because"
	...
	if (st == NULL)
	{
	    ...
	    if (st == NULL)
	    {
		plog("[Tunnel Authorize Fail] Quick Mode message is for a non-existent (expired?)"
		    " ISAKMP SA");
		/* XXX Could send notification back */
		return;
	    }
	    ...
	    if (!IS_ISAKMP_SA_ESTABLISHED(st->st_state))
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Quick Mode message is unacceptable because"
	    ...
	    if (!unique_msgid(st, md->hdr.isa_msgid))
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Quick Mode I1 message is unacceptable because"
	...
	if (is_zero_cookie(md->hdr.isa_icookie))
	{
	    plog("[Tunnel Authorize Fail] Mode Config message is invalid because"
		" it has an Initiator Cookie of 0");
	    /* XXX Could send notification back */
	    return;
	}
	...
	if (st == NULL)
	{
	    ...
	    if (st == NULL)
	    {
		plog("[Tunnel Authorize Fail] Mode Config message is for a non-existent (expired?)"
		    " ISAKMP SA");
		/* XXX Could send notification back */
		return;
	    }
	    ...
	    if (!IS_ISAKMP_SA_ESTABLISHED(st->st_state))
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] Mode Config message is unacceptable because"
		...
		plog("[Tunnel Authorize Fail] received MODECFG message when in state %s, and we aren't mode config client"
		     , enum_name(&state_names, st->st_state));
		return;
	    }
	}
    ...
    default:
	plog("[Tunnel Authorize Fail] unsupported exchange type %s in message"
    ...
    {
	loglog(RC_LOG, "[Tunnel Authorize Fail] discarding packet received during DNS lookup in %s"
	    , enum_name(&state_names, st->st_state));
	return;
    }
	...
	if (smc->flags & SMF_RETRANSMIT_ON_DUPLICATE)
	{
		...
		loglog(RC_RETRANSMISSION
		    , "[Tunnel Authorize Fail] retransmitting in response to duplicate packet; already %s"
		    , enum_name(&state_names, st->st_state));
		send_packet(st, "retransmit in response to duplicate");
	    }
	    else
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Fail] discarding duplicate packet -- exhausted retransmission; already %s"
		    , enum_name(&state_names, st->st_state));
	    }
	}
	else
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Negotiation Fail] discarding duplicate packet; already %s"
		, enum_name(&state_names, st->st_state));
	}
	return;
    }
	...
	if (st == NULL)
	{
	    plog("[Tunnel Authorize Fail] discarding encrypted message for an unknown ISAKMP SA");
	    SEND_NOTIFICATION(PAYLOAD_MALFORMED /* XXX ? */);
	    return;
	}
	if (st->st_skeyid_e.ptr == (u_char *) NULL)
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] discarding encrypted message,"
	 ...
	 */
	{
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] malformed message: not a multiple of encryption blocksize");
		SEND_NOTIFICATION(PAYLOAD_MALFORMED);
		return;
	    }
	...
	if (smc->flags & SMF_INPUT_ENCRYPTED)
	{
	   ...
	   {	
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] packet rejected: should have been encrypted");
		SEND_NOTIFICATION(INVALID_FLAGS);
		return;
	   }
	}
	...
	while (np != ISAKMP_NEXT_NONE)
	{
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] more than %d payloads in message; ignored", PAYLIMIT);
		SEND_NOTIFICATION(PAYLOAD_MALFORMED);
		return;
	    }
		...
		case ISAKMP_NEXT_NATOA_DRAFTS:
		    ...
		    break;
		default:
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] %smessage ignored because it contains an unknown or"
		...
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] %smessage ignored because it "
	    ...
	    {
		loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] %smalformed payload in packet", excuse);
	...
	if (needed != 0)
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] message for %s is missing payloads %s"
	...
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] malformed Phase 1 message: does not start with an SA payload");
	    SEND_NOTIFICATION(PAYLOAD_MALFORMED);
	    return;
	}
    }
	...
	{
	    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] malformed Quick Mode message: does not start with a HASH payload");
	    SEND_NOTIFICATION(PAYLOAD_MALFORMED);
	    return;
	}

	{
		...
		if (p != &md->digest[i])
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] malformed Quick Mode message: SA payload is in wrong position");
		    SEND_NOTIFICATION(PAYLOAD_MALFORMED);
		    return;
		}
	    }
	 ...
	 */
	{
		...
		{
		    loglog(RC_LOG_SERIOUS, "[Tunnel Authorize Fail] malformed Quick Mode message:"
...
complete_state_transition(struct msg_digest **mdp, stf_status result)
{
	...
	case STF_OK:
           ...
           * from STATE_AGGR_R1 to STATE_AGGR_R2 */
           if(nat_traversal_enabled && (st->st_policy&POLICY_NATTRAVERSAL))
           {
...
//<<
		...
		{
		    /* log our success */
		    plog("[Tunnel Established] %s%s}", story, sadetails);	
		    w = RC_SUCCESS;
		}
