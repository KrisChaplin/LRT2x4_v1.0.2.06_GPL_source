...
/*purpose     : Service Management author : jason.huang date : 2011-04-15 */
/*description : Move to nkutil.h                                          */
// typedef struct {
...
kdsys_printf("set_config start !!");
	...
	if ( 0 == (result = find_category(database, category)))
	{
		...
		{
		   /* purpose     : 0015939    author : paul.chen    date : 2012-07-13 */
		   /* description : Do not do case conversion                          */
		   /*
...
static void print_item_value(FILE *handle)
{
   if ( 0 == get_item_value (handle))
   {
     if ( reqBuf == NULL )
       ...
       strcpy(reqBuf, glvalue);  //sprintf(reqBuf, glvalue);
...
//					kd_Log("fail to create %s\n",dhcpconf);
	...
	{
		sprintf(opcode, "ISP%d", wanidx+1);
		if ( !find_category(database, opcode) && !find_item(database, "GATEWAY_V6") )
		{
			get_item_value(database);
			if ( !strcmp(glvalue, "::") )
			{
		...
		if ( 0 == ( result = find_category(database, opcode)))
		{
			if ( !find_item(database, "USERSPECIALDNS_V6") &&  !get_item_value(database) )
			{
				if ( !strcmp(glvalue, "NO") )	// not USERSPECIALDNS, ISP DNS
					sprintf(opcode, "ISP%d", wanidx+1);
			}
		...
		if ( 0 == ( result = find_category(database, opcode)))
		{
			if ( 0 == ( result = find_item(database, "DNS1_V6"))) 
			{
				result = get_item_value(database);
				if ( result == 0) 
				{
...
#endif
	{
		...
		if ( 0 == ( result = find_category(database, opcode)))
		{
			if ( 0 == ( result = find_item(database, "DNS1_V6"))) 
			{
				...
				if ( result == 0) 
				{
					if ( 0 == (result = find_item(database, "DNS2_V6"))) 
					{
						result = get_item_value(database);
						if ( result == 0 ) 
						{
...
static int create_dhcp_config(void)
{
	...
	char lanIp[40];
	int  wanidx, index, result, isp_gwno;
	FILE *rfd;
	char opcode[60];
	int dnsidx, dnsNum, dns_type = 0;
	char dnslist[4][20];
#ifdef CONFIG_NK_DYNAMIC_PORT_NUM
	unsigned int DYNAMIC_NUM_WAN = Get_Num_Wan_kdsys();
#endif
	...
	if (strcmp(glvalue, "0"))
	{
/*purpose     : 0017124 author : tim.tsai date : 2013-04-17 */
/*description : cancel feature : no wan with 5 mins         */
//		if (isp_gwno == 0)
...
//		else
	...
	if ( !find_category(database, "DHCP") && !find_item(database, "DNSSERVER") && !get_item_value(database))
	{
		dnsNum = 0;
		fprintf(wfd, "option  dns       ");
		dns_type=atoi(glvalue);
		switch(dns_type)
		{
			case 1:	//Use DNS Proxy
				if ( !find_category(database, "SYSTEM") && !find_item(database, "LAN") && !get_item_value(database))
				{
					fprintf(wfd, "%s", glvalue);
				}
				break;
			case 2:	//Use DNS from ISP
				...
				for (dnsidx = 0; dnsidx < 3; dnsidx++) {
...
#endif
					{
						sprintf(opcode, "ISP%d", wanidx+1);			
						if (!find_category(database, opcode) && !find_item(database, "GATEWAY")) {
							if (get_item_value(database) || !strcmp(glvalue, "0.0.0.0"))
								continue;
						}

						sprintf(opcode, "WAN%d", wanidx+1);
						if (!find_category(database, opcode)) {
							if (!find_item(database, "USERSPECIALDNS") &&  !get_item_value(database)) {
								if (!strcmp(glvalue, "NO"))	// not USERSPECIALDNS, ISP DNS
									sprintf(opcode, "ISP%d", wanidx+1);
							}
				...
				break;
			case 3:	//Use DNS as Below
				if ( !find_category(database, "DHCP") && !find_item(database, "DNS1") && !get_item_value(database))
				{
					if (strcmp(glvalue, "0.0.0.0"))
					{
						fprintf(wfd, "%s ", glvalue);
				...
				if( !find_item(database, "DNS2") && !get_item_value(database))
				{
					if (strcmp(glvalue, "0.0.0.0"))
					{
						fprintf(wfd, "%s", glvalue);
...
/*purpose     : add DHCP to DNS BD  author : Max date : 2011-07-25 */
/*description : add DHCP to DNS BD                   */
/*	fprintf(wfd, "auto_time         900\n");*/
	fprintf(wfd, "auto_time         60\n");
	fclose(wfd);
	fclose(database);
...
//printf ("%d result %d\n", __LINE__, result);
		...
		(!(strcmp (type, "AUTOBALANCE_RULE"))) ||(!(strcmp (type, "AGING_TIME"))) || (!(strcmp (type, "8021Q_SIMPLE_LAN"))) ||(!(strcmp (type, "OPENVPN_CLIENT"))) ||
		...
		(!(strcmp (type, "DDNS1"))) ||(!(strcmp (type, "DDNS2"))) ||(!(strcmp (type, "DDNS3"))) ||(!(strcmp (type, "DDNS4"))) ||(!(strcmp (type, "DDNS5"))) ||(!(strcmp (type, "DDNS6"))) ||(!(strcmp (type, "DDNS7"))) ||(!(strcmp (type, "DDNS8"))) ||
...
#ifdef CONFIG_NK_EZLINKVPN
		|| (!(strcmp (type, "EZLINKVPN")))
#endif	
...
//		if (!strncasecmp (type, "USER", 4))
	...
	else if (!(strcmp (type, "DHCP")) && !(strcmp (opcode, "RANGE")))
	{
		...
		(!(strcmp (type, "BLACKLIST"))) || (!(strcmp (type, "WHITELIST"))) ||(!(strcmp (type, "AGING_TIME"))) || (!(strcmp (type, "8021Q_SIMPLE_LAN")))|| (!(strcmp (type, "OPENVPN_CLIENT")))||
...
/*purpose : UI_IMPROVE author : jason.huang reviewer : Dio.Li date : 2011-09-20*/
/*description : Add DB read size , buf and path            */
char* ReplaceValue(char *string,char *varname,char *var) // string : soure string ; varname : search Item ; var: replace value ;
{
    char *ptr;
    char  str1[1024];
    char  str2[1024];
    ...
    char  databuf[1024];
    int   i;
    if((strlen(varname) > strlen(string))||(strlen(string)<=0))
    {
        return string;
    }
    sprintf(var1,"\"%s\"",var);
    strcpy(str1,string);
    ptr = strstr(str1,varname);
    if(ptr==NULL)
    {
        return string;  //No varname exit
    }
    strcpy(str2,ptr);
    *ptr='\0';
    for (i=0; (str2[i] != '&'); i++);
    sprintf(databuf,"%s%s=%s%s",str1,varname,var1,str2+i);
    return databuf;
}
...
/*purpose     : Service Management author : jason.huang date : 2011-04-15 */
/*description : Update Service Management DB                              */
db_value_get *BatchGet_config_m(char *field, char *subfield, db_value_get *db_list)
{
    FILE  *srcFile;
    ...
    {
        srand(time(0));
        j = 100 + (int) (120.0 * rand()/(RAND_MAX + 1.0));
        for (l = 0; l < j; l ++);
        fd = open (FlagFile, O_RDONLY | O_CREAT | O_EXCL, 0666);
    }
    fileLine[0]='\0';
            ...
            if(!strcmp(tmp,field))
            {
                ...
                while(fgets(fileLine,LENGTH_READ_SIZE,srcFile) != NULL)
                {
                    ...
                    if(!strcmp(tmp,subfield))
                    {
                        ...
                        entry[sidx]='\0';
                        strcpy(db_list[snumber].entry,entry);
                        snumber++;
                    }
                }
            }
