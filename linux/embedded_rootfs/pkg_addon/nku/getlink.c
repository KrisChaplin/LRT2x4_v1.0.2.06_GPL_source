...
unsigned int wanportmap(unsigned int wanport, unsigned int switch_type) {
	...
	else if ( switch_type == 214 || switch_type == 224 || switch_type == 42 || switch_type == 422 )
...
// 			else
        ...
        else if(uLedNo == NK_VPN_CONN_LED)
        {
            nk_set_led(NK_VPN_CONN_LED, LED_ON);
        }
...
// 			else
        ...
        {
            nk_set_led(NK_VPN_CONN_LED, LED_OFF);
        }
		else if(uLedNo == NK_DIAG_LED)
		{
			nk_set_led(NK_DIAG_LED, LED_OFF);
...
void nk_set_led(unsigned int uLedNo, int state)
{
	...
	if(uLedNo != NK_DMZ_CONN_LED && uLedNo != NK_DIAG_LED && uLedNo != NK_VPN_CONN_LED )
	{
...
int NK_GET_PORT_STATUS(struct PortStatusNStatistics *status) {
    ...
    KdDoCmdPrint("SYSTEM NEW_NUM_WAN", ui_wan_num);    
...
int NK_SET_VLAN_GROUP(void) {
    ...
    if ( SWITCH_TYPE == 214 || SWITCH_TYPE == 224)
    {
        return 0;
    }    
#endif  
...
void Show_Recieve_Lan(void)
{
	...
	{
		console_printf2("%s: Open /dev/eth_acc Fail!!!(fd=%d)\n", __func__, fd);
		return;
	}
...
void nk_hw_support_mac_clone ( void )
{
    int fd;
    char buf[200] = { 0 };
    uint64_t mac = 0;

    NK_db_read ( "SYSTEM LANMAC", buf );
    sscanf ( buf, "%lx", &mac );
    
    if ( ( fd = open ( "/dev/nk_switch", O_RDONLY ) ) < 0 )
    {
        console_printf ( "open /dev/nk_switch failed!!!\n" );
        return -1;
    }

    if ( ioctl ( fd, NK_HW_SUPPORT_MACCLONE, &mac ) != 0 )
    {
        con_printf ( "ioctl NK_HW_SUPPORT_MACCLONE failed!!!\n" );
        close ( fd );
        return -2;
    }
