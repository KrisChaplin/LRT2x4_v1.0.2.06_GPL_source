...
//static struct hostent *ipHostent = NULL;
...
int ping(const char *host, const char *SourceIp ,  struct hostent *h )
{
	//struct hostent *h;
	int count;
	fd_set set;
	...
	if( (Source.sin_addr.s_addr = inet_addr(SourceIp)) == -1 ) {
		printf("NSD Ping2: SourceIp inet_addr() transfer error\n");
		res=-1;
		goto exit;
	}
	if( bind(pingsock, (struct sockaddr *)&Source,sizeof(Source)) == -1) {
		printf("NSD Ping2: bind error\n");
		res=-1;
		goto exit;
	}
	...
	{
		printf("NSD Ping2: pingaddr inet_addr() transfer error\n");
		//printf("\n====== pingaddr.sin_addr.s_addr=%x   inet_addr(host)=%x  host=%s\n",pingaddr.sin_addr.s_addr,inet_addr(host) , host);

		//h = timeGethostbyname(host,2);
		if(h)
		{
			//pingaddr.sin_addr.s_addr = *((u_long  *) (h->h_addr));
			hostname = h->h_name;
			memcpy(&pingaddr.sin_addr.s_addr, h->h_addr, sizeof(pingaddr.sin_addr));
			//sprintf(buff,"\n==h-name =%s  h-h_addr=%x  Source.sin_addr.s_addr=%x\n" ,h->h_name ,pingaddr.sin_addr.s_addr,Source.sin_addr.s_addr );
	...
	trenchen_printf("receive error2\n");
	res = -1;
exit :
	close(pingsock);
	return res;
