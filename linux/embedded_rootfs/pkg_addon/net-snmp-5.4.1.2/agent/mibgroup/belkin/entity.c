	...
	switch(vp->magic)
	{
	  case entPhysicalDescr:
		{
			...
			/* purpose      :  RV042G  author :  Paul.Chen  date :  2011-11-29         */
			/* description  :  Add RV042G entPhysicalDescr                             */
			kd_doCommand("VERSION MODEL", CMD_PRINT, ASH_DO_NOTHING, DBdata);
			if (!strcmp("RV042G", DBdata))
			{
				sprintf(tmp, "RV042G Gigabit Dual WAN VPN Router");
			}
			else if (!strcmp("LRT214", DBdata))
			{
				sprintf(tmp, "LRT214 Gigabit VPN Router");
			}
			else if (!strcmp("LRT224", DBdata))
			{
				sprintf(tmp, "LRT224 Dual WAN Gigabit VPN Router");
			}
			else if (!strcmp("RV042", DBdata))
			{
				sprintf(tmp, "10/100 4-Port VPN Router");
			}
			else if (!strcmp("RV082", DBdata))
			{
				sprintf(tmp, "10/100 8-Port VPN Router");
			}
			else if (!strcmp("RV016", DBdata))
			{
      ...
      *var_len = strlen(tmp);
      return (u_char *) tmp;
		...
		#else
			/* purpose      :  RV042G  author :  Paul.Chen  date :  2011-11-29         */
			/* description  :  Add RV042G entPhysicalHardwareRev                       */
			kd_doCommand("VERSION MODEL", CMD_PRINT, ASH_DO_NOTHING, DBdata);
			if (!strcmp("RV042G", DBdata))
			{
				strcpy(tmp, "V01");
